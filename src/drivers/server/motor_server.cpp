 ...
 *
 *   Motor API:
 *     - Thruster(int pin, int handle)   -- constructor claims pin; destructor
 * frees it
 *     - motor.armMotor()                -- blocks ~3 s, sets neutral
 *     - motor.setSpeed(float speed)     -- -1.0 (full reverse) .. 0.0 (stop)
 * .. 1.0 (full forward)
 *
...
// Current normalized speed for each motor (source of truth for "status"
// replies)
static std::array<float, NUM_MOTORS> g_speeds = {STOP_SPEED, STOP_SPEED,
...
static bool parse_int(const std::string &json, const std::string &key,
                      int &out) {
  std::string search = "\"" + key + "\"";
  size_t pos = json.find(search);
  if (pos == std::string::npos)
...
static float pwm_to_speed(int pwm) {
  return static_cast<float>(pwm - NEUTRAL_PWM) /
         static_cast<float>(MAX_PWM_DIST);
}
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < NUM_MOTORS; i++) {
    try {
      ...
      std::cerr << "[INIT] Failed to construct motor " << i << ": " << e.what()
                << std::endl;
#ifndef BUILD_SIMULATION
  ...
  std::array<Motor *, NUM_MOTORS> motors = {
      motor_storage[0].get(), motor_storage[1].get(), motor_storage[2].get(),
      motor_storage[3].get()};
      ...
      if (cmd == "stop") {
        ...
        for (int i = 0; i < NUM_MOTORS; i++) {
          ...
          if (motors[i]->setSpeed(speed) < 0) {
            server.send_line(make_response(false, "motor " + std::to_string(i) +
                                                      " setSpeed failed"));
            ok = false;
            break;
          }
        ...
        if (ok)
          server.send_line(make_response(true, "motors updated"));
        ...
        if (!parse_int(line, "motor", motor_idx) ||
            !parse_int(line, "value", pwm_value) || motor_idx < 0 ||
            motor_idx >= NUM_MOTORS || pwm_value < MIN_PWM ||
            pwm_value > MAX_PWM) {
          server.send_line(
              make_response(false, "bad args -- motor 0-3, value 1100-1900"));
          continue;
        }
        ...
        server.send_line(
            make_response(true, "motor " + std::to_string(motor_idx) + " -> " +
        ...
        server.send_line(make_response(
            false, "unknown command: " + (cmd.empty() ? "(empty)" : cmd)));
      }
    }
