...
SimulatedMotor::SimulatedMotor(SimulatedMotor::MotorLocation ml, int speed, float cycle) : Motor(speed, cycle)
{
...
int SimulatedMotor::setSpeed(float speed) 
{
    ...
    auto out = SimulatedMotor::sim_connection->Post(this->GetMotorPath().c_str(),  s.c_str(),"application/json");
    if(out == nullptr){
