...
Thruster::Thruster(int pin, int handle, int speed /*= 0*/, int cycle /*= 50*/)
    : pin(pin), handle(handle), Motor(speed, cycle) {
...
int Thruster::sendMotorPWM() {
  if (!this->armed) {
    throw new std::logic_error(
        "Tried to affect motor before arming. Always ensure motors are armed "
